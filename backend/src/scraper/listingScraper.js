...
// Poster images on Scope's site are lazy-loaded: the real URL isn't
// guaranteed to be swapped into img.src until the card has scrolled into
// view AND the lazy-load JS has had time to run. Scrolling to the bottom
// gets every <article> into the DOM, but doesn't guarantee every image's
// swap has finished by the time we're done scrolling — that's a separate
// race. This waits (with a bounded timeout) for every poster <img> to
// report itself fully loaded before we read anything out of the DOM,
// so we don't grab a stale/placeholder src for cards that just hadn't
// finished loading yet.
async function waitForAllPostersLoaded(page, { timeout = 15_000 } = {}) {
  try {
    ...
    logger.warn('Timed out waiting for all poster images to finish loading — proceeding with whatever loaded so far');
  }
}
...
export async function scrapeMovieListing() {
  ...
  for (const pageUrl of LISTING_PAGES) {
    ...
    try {
      ...
      await waitForAllPostersLoaded(page);

      const movies = await page.$$eval('article', (articles) => {
        return articles
          .map((article) => {
            // Poster — prefer currentSrc, which reflects the actually
            // resolved image regardless of which lazy-load technique
            // swapped it in. Fall back to src, then any common lazy-load
            // data attribute, for images that still hadn't resolved.
